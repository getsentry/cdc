...
def test(mock_incr, mock_timing) -> None:
    ...
    mock_incr.assert_called_with(Stats.MESSAGE_WRITTEN_METRIC, tags=None, sample_rate=0.1)

    stats.message_flushed(10)
    mock_timing.assert_called_with(Stats.MESSAGE_FLUSHED_METRIC, ANY,
        tags=None, sample_rate=0.1)

    stats.task_executed(10, "task")
    mock_timing.assert_called_with(Stats.TASK_EXECUTED_TIME_METRIC, ANY,
        tags=["tasktype:task"], sample_rate=1)
